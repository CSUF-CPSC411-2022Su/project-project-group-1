...
//  CarbonFoodPrint
//
//  Created by Nguyen, Randy on 6/28/22.
//
import Foundation
import SwiftUI

extension UIImage {
    func circle(byDrawingImage image: UIImage, inRect rect: CGRect) -> UIImage! {
...
struct MapsAPIResult: Codable {
    var type: String
    var query: [String]
    var features: [GreenMapsArea]
    // TODO: Provide a property to represent attribution
    var attribution: String
}

struct GreenMapsArea: Codable {
    var text: String
...
class GreenMapsFinder: ObservableObject {
    @Published var firstFoundName = ""
    @Published var image = UIImage()

    private var apiKey = "pk.eyJ1IjoicGludmVudGFkbyIsImEiOiJjbDFreTYwZWwwNWY3M2JvZm1ieDNpOGVsIn0._iXvNRhRLnJEYoLGzzY5IQ"
    ...
    func find(_ searchString: String) {
        ...
        let mapboxSearchURL = "https://api.mapbox.com/geocoding/v5/mapbox.places/\(searchString).json?access_token=\(apiKey)"
    ...
    func loadMapImage(long: Double, lat: Double) {
        let zoom = 12
        let rotation = 0
        let mapboxImageURL = "https://api.mapbox.com/styles/v1/mapbox/streets-v11/static/\(long),\(lat),\(zoom),\(rotation)/300x200?access_token=\(apiKey)"
